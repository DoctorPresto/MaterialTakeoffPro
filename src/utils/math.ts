...
export const getSlopeMultiplier = (pitch: number): number => {
    // Hypotenuse of triangle with run 12 and rise {pitch}
    return Math.sqrt(Math.pow(pitch, 2) + 144) / 12;
};

export const getPlanHipToTrueHipMultiplier = (pitch: number): number => {
    // 288 is 12^2 + 12^2 (The squared length of the plan hip run for a 12" unit)
    const planHipRunSquared = 288;
    const trueHipLength = Math.sqrt(planHipRunSquared + Math.pow(pitch, 2));
    const planHipRun = Math.sqrt(planHipRunSquared); // ~16.97
    return trueHipLength / planHipRun;
};
